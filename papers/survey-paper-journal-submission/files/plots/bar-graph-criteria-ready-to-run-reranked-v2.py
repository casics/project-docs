...
data = sorted(raw_data.items(), key=lambda x: x[1][0]+x[1][1]+x[1][2], reverse=True)
...
# plt.title('\emph{Responses to the question "In general, how important are the following characteristics}\n\emph{when you are searching for ready-to-run software for a task?}',
...
for rect, v1, v2, v3 in zip(h_essential.patches, essential, aboveavg, avg):
    percentage = 100*(v1 + v2 + v3)/total_responses
    text = '({: >2.0f}\%)'.format(percentage)
    plt.gca().text(75,
...
plt.savefig('bar-graph-criteria-ready-to-run-reranked-v2.pdf', bbox_inches='tight')
plt.close()
